...
def process_file_to_log(call_param):
    ...
    print "add job {0}, trace:{1}, digest:{2}, exec time:{3}, unsigned:{4}:".format(
        filename, trace, digest, exec_time, unsigned
    )
    print "init risc 16"
    try:
        risc = risc16.RISC16(
            trace=trace, digest=digest, logfile=fo, IS=archi, unsigned=unsigned
        )
...
def serve_reports(filename):
    ...
    if os.access(outf, os.R_OK):
        ...
        return render_template(
            "log_done.html", filename=filename, log_content=log, status="download"
        )
    else:
        return abort(404)
...
def serve_code(filename):
    ...
    return send_from_directory(
        UPLOAD_FOLDER, filename=filename, mimetype="text/plain", as_attachment=True
    )
...
def log_page(filename):
    ...
    if os.access(outf, os.R_OK):
        ...
        filename = filename.rsplit(".", 1)[0]  # strip the extension
        return render_template(
            "log_done.html", filename=filename, log_content=log, status="done"
        )
        ...
        # print log
        return render_template(
            "log_done.html", filename=filename, log_content=log, status="pending"
        )
    else:
        print "File does not exist, wait...", os.path.join(LOG_FOLDER, filename),
        # pending=pool._taskqueue.qsize()
...
def upload():
    ...
    if request.method == "POST":
        ...
        if (
            ...
            and file.content_length < app.config["MAX_CONTENT_LENGTH"]
            and jobs_pending < app.config["MAX_JOBS_WAITING"]
        ):
            print "loading..."
            ...
            filename = (
                ...
                + rand
                + "-"
                + get_date_fmt_file(datetime.datetime.now())
                + "."
                + ext
            )
            filename_log = (
            ...
            call_param = {
                "filename": filename,
                "digest": request.form.has_key("asm_digest"),
                "trace": request.form.has_key("trace"),
                "exec": exec_instr,
                "test_file": test_file,
                "archi": archi,
                "unsigned": unsigned,
...
def admin():
    ...
    if ip == "127.0.0.1":
        ...
        for f in all_files:
            ...
            code_name = """<a href="{1}" download title="Download code">{0}.txt</a> <a href="{1}"  title="See code">see</a>""".format(
                filename, url_for("serve_code_html", filename=filename + ".txt")
            )
            log_name = os.path.join(LOG_FOLDER, filename + ".log")
            log_name = """<a href="{0}" title="Report page">report</a>""".format(
                url_for("log_page", filename=filename + ".log")
            )
